...
%>
    <div style="text-align: center; margin-bottom: 20px;">
        <p style="color: #856404; background-color: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; border-radius: 5px; display: inline-block;">
            ⚠️ Hai <%= prodottiEliminati %> prodotto<%= prodottiEliminati > 1 ? "i" : "" %> non più disponibile<%= prodottiEliminati > 1 ? "i" : "" %> nei preferiti
        </p>
        <br>
        <form action="<%= request.getContextPath() %>/favorites" method="post" style="display: inline;">
            <input type="hidden" name="action" value="cleanup">
            <button type="submit" class="cleanup-button">
                🧹 Rimuovi tutti i prodotti non disponibili
            </button>
        </form>
    </div>
<%
...
%>
    ...
    <% for (ProductBean bean : products) { %>
        <div class="element-card <%= bean.isEliminato() ? "prodotto-eliminato" : "prodotto-disponibile" %>">
            ...
            <% if (bean.isEliminato()) { %>
                <div class="warning-message">
                    ⚠️ PRODOTTO NON PIÙ DISPONIBILE
                </div>
                <p style="color: #6c757d; font-style: italic;">
                    Questo prodotto è stato rimosso dal catalogo
                </p>
            <% } else { %>
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 15px;">
                ...
                <form action="<%= request.getContextPath() %>/favorites" method="post" style="display: inline;">
                    ...
                    <button type="submit" style="background:none; border:none; padding:0; margin:0; line-height:0; font-size:0; width:30px; height:30px; cursor:pointer;" title="Rimuovi dai preferiti">
                ...
                <% if (!bean.isEliminato()) { %>
                    <a href="<%= request.getContextPath() + "/order?productId=" + bean.getIdProdotto() %>" title="Visualizza prodotto">
                        <img src="<%= request.getContextPath() %>/images/lente.png" alt="Visualizza" style="width:40px; height:40px;">
                    </a>
                <% } else { %>
                    <span style="color: #6c757d; font-style: italic;">Non visualizzabile</span>
                <% } %>
            </div>
        </div>
    <% } %>
