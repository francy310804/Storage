<%@ page
	import="java.sql.*, it.unisa.order.*, java.util.*, it.unisa.product.*"%>
<%
...
%>
	...
	<table class="product-table">
		<tr>
			...
			<br> <a href="product?action=add&id=<%=bean.getIdProdotto()%>">
					<img src="<%= request.getContextPath() + "/images/chart.jpg"%>"
					alt="chart"
	...
	<div style="width: 80%; margin: 20px auto;">
		...
		<form action="OrderControl?action=review" method="post">
			...
			<div class="star-rating">
				...
				<label for="star5" title="5 stelle">&#9733;</label> <input
					type="radio" id="star4" name="rating" value="4" /> <label
					for="star4" title="4 stelle">&#9733;</label> <input type="radio"
					id="star3" name="rating" value="3" /> <label for="star3"
					title="3 stelle">&#9733;</label> <input type="radio" id="star2"
					name="rating" value="2" /> <label for="star2" title="2 stelle">&#9733;</label>

				<input type="radio" id="star1" name="rating" value="1" /> <label
					for="star1" title="1 stella">&#9733;</label>
			</div>

			<br>
			<br> <label for="comment">Commento:</label><br>
    ...
    %>
		<div class="review-card">
			<div class="review-header">
				<span class="review-stars"> <% for (int i = 1; i <= review.getRating(); i++) { %>
					<span>&#9733;</span> <% } %> <% for (int i = review.getRating() + 1; i <= 5; i++) { %>
